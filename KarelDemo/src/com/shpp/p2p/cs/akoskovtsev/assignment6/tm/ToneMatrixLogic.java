package com.shpp.p2p.cs.akoskovtsev.assignment6.tm;
...
public class ToneMatrixLogic {
    ...
    public static double[] matrixToMusic(boolean[][] toneMatrix, int column, double[][] samples) {
        ...
        for (int row = 0; row < toneMatrix.length; row++) {
            if (toneMatrix[row][column]) {
                // if the row is activated, add the corresponding array of sample to the list
    ...
    private static double getMaxOfAmplitude(double[] unnormalizedSamples) {
        double max = 0.0;
        double min = 0.0;
        for (int i = 0; i < unnormalizedSamples.length; i++) {
            max = Math.max(max, unnormalizedSamples[i]);
            min = Math.min(min, unnormalizedSamples[i]);
        }
        double minAbs = -min;
        // If the minimum value is negative, we need to consider its absolute value
        if (minAbs > max) {
            max = min;
        }
        return max == 0 ? 1 : max;
