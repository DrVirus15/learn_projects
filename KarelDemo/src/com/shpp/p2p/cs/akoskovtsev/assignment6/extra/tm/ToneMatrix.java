...
//            this.loadMatrix();
            System.out.println("Loading matrices is not implemented in this version. " +
                    "Please use the save functionality only.");
        }
...
//            try {
//                // new version
//                this.display.getMatrix(this.readMatrixFromFile(fc.getSelectedFile().getAbsolutePath()));
//                /// (oldVersion)this.display.setMatrix(this.readMatrixFromFile(fc.getSelectedFile().getAbsolutePath()));
...
//    }
    ...
    public void writeMatrixToFile(int[][] toneMatrix, String fileName) {
        try {
    ...
    public int[][] readMatrixFromFile(String fileName) {
        try {
            int[][] e = new int[MATRIX_SIZE][MATRIX_SIZE];
            ...
            for (int row = 0; row < MATRIX_SIZE; ++row) {
                for (int col = 0; col < MATRIX_SIZE; ++col) {
                    ...
                    // new version
                    e[row][col] = Integer.parseInt(br.readLine());

                    /// (oldVersion)  e[row][col] = Boolean.parseBoolean(br.readLine());
                }
            }
    ...
    public void run() {
        double[][] samplesTones = ToneMatrixWaveGenerator.getSamples();
        double[][] samplesBasses = BasesMatrixWaveGenerator.getSamples();
        double[][] samplesGuitar = GuitarMatrixWaveGenerator.getSamples();

        try {
            while (true) {
                for (int e = 0; e < MATRIX_SIZE; ++e) {
                    int[][] matrix = this.display.getMatrix();
                    this.display.highlightColumn(e);
                    double[] toPlay = ToneMatrixLogic.matrixToMusic(matrix, e, samplesTones, samplesBasses, samplesGuitar);
