package orig.src.com.shpp.p2p.cs.akoskovtsev.assignment13;
...
public class Eraser {
    ...
    public boolean[][] separateSilhouettesMask() {
        ...
        boolean[][] erasedImageBgMask = new boolean[image.getHeight()][image.getWidth()];
    ...
    private boolean[][] findBackgroundMask() {
        boolean[][] backgroundMask = new boolean[image.getHeight()][image.getWidth()];
        for (int row = 0; row < image.getHeight(); row++) {
            for (int col = 0; col < image.getWidth(); col++) {
