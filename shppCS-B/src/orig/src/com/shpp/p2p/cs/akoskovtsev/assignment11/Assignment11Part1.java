package orig.src.com.shpp.p2p.cs.akoskovtsev.assignment11;

import orig.src.com.shpp.p2p.cs.akoskovtsev.assignment11.operators.*;
...
//        String formula = "15/(7-(1+1))*3-(2+(1+1))*15/(7-(200+1))*3-(2+(1+1))*(15/(7-(1+1))*3-(2+(1+1))+15/(7-(1+1))*3-(2+(1+1)))";
     ...
     */
    private static double calculateRPN(LinkedList<String> rpn, HashMap<String, Double> variables) throws NumberFormatException {
        Stack<Double> tokens = new Stack<>();
        setVariables(variables, rpn);     // Replace variables with their values
        while (!rpn.isEmpty()) {
